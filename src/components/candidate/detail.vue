  ...
  <div class="candidateDetail">
    ...
    <div v-if="Object.keys(pageData.detail).length > 0">
      <div class="base">
        <div class="df">
          <div class="avatar">
            <img v-bind:src="pageData.avatar" alt="">
          </div>
          <div class="f1">
            <p class="name clearfix">
              {{pageData.name}}<span class="id">(ID:{{basicData.PERFILE_NO}})</span>
              <mt-badge type="primary" size="small" class="fr">{{basicData.TYPE}}</mt-badge>
            </p>
            ...
            <p>{{new Date().getFullYear() - (getValue('work_begin_year', 'basic'))}}年经验 · 现居{{pageData.detail.city_name}}</p>
          </div>
        </div>
      ...
      <div class="info">
        <ul>
          <li>
            ...
            <dl>
              <dd v-for="item in pageData.detail.jobs">
                <p class="companyName">{{item.company.value}}</p>
                <p class="companyDate">
                  {{item.start_date.value.replace(/-/g, '/')}} - {{item.end_date.value === '0000-00-00' ? '至今' : item.end_date.value.replace(/-/g, '/')}}
          ...
          <li>
            ...
            <dl>
              <dd v-for="item in pageData.detail.edus">
      ...
      <div class="feedback">
        <ul>
          <li v-for="item in interviewerFeedbacks">
            <p>面试反馈：{{item.content}}</p>
  ...
  export default {
    ...
    methods: {
      ...
      diffDate(startDate, endDate) {
        return moment(endDate === '0000-00-00' ? new Date() : endDate).diff(moment(startDate), 'years', true).toFixed(1);
      },
      // 添加面试反馈
