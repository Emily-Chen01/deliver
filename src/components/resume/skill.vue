<template>
  <div>
    <div class="skilling module" v-show="skillNumber">
      <div class="interlayer"></div>
      <div class="title">{{upData}}</div>
      <div class="box" v-for="(item,i) in SkillList" v-show="SkillList.length>0" :key="i">
        <div class="info" style="width:82%;">
          <div class="info-item" v-show="item.SkillName||item.SkillName==''">技能名称：{{item.SkillName || "-"}}</div>
          <div class="info-item" v-show="item.SkillType||item.SkillType==''">技能类型：{{item.SkillType || "-"}}</div>
          <div class="info-item" v-show="item.SkillLevel1||item.SkillLevel1==''">技能等级：{{item.SkillLevel1 || "-"}}</div>
        ...
        <div class="work-action module-action">
          <div class="edit" @click="toEditSkill(item)">
            <div class="icon"></div>
          </div>
          <div class="delete" @click="toDeleteSkill(item)">
            <div class="icon"></div>
          </div>
        </div>
      </div>
      <div class="add">
        <div class="add-link" @click="toAddSkill">
    ...
    toDeleteSkill(item) {
      Dialog.confirm({
        ...
        .then(() => {
          this.$http
            .get(
              `/api/QR/DeleteUserData?userId=${sessionStorage.getItem('userId')}&category=skillexp&order=${item.order}`
            )
            .then(response => {
              let res = response.body;
              if (res.code === "200") {
                // Toast.success("删除成功");
                this.$emit('skillQuery');
              } else {
                Toast.fail("删除失败");
              }
            });
