<template>
  <div>
    <div class="practing module" v-show="practNumber">
      <div class="interlayer"></div>
      <div class="title">{{upData}}</div>
      <div class="box" v-for="(item,i) in PractList" v-show="PractList.length>0" :key="i">
        <div class="info" style="width:82%;">
          <div class="info-item" v-show="item.CompanyName||item.CompanyName==''">单位名称：{{item.CompanyName || "-"}}</div>
          <div class="info-item" v-show="item.PositionName||item.PositionName==''">职位：{{item.PositionName || "-"}}</div>
    ...
    toDeletepracting(item) {
      Dialog.confirm({
        ...
        .then(() => {
          this.$http
            .get(
              `/api/QR/DeleteUserData?userId=${sessionStorage.getItem('userId')}&category=practexp&order=${item.order}`
            )
            .then(response => {
              let res = response.body;
              if (res.code === "200") {
                // Toast.success("删除成功");
                this.$emit('practQuery');
              } else {
                Toast.fail("删除失败");
              }
            });
